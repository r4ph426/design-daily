...
- GitHub Actions runs the crawl every day at 06:17 in `Europe/Berlin`.
...
- Newsletter suggestions arrive as GitHub issues and are included automatically in the next crawl.
...
Curated feeds are listed in `data/sources.json`. Each entry can include a homepage URL, a direct feed URL, one of the three categories, and UI or UX tags. Open GitHub issues whose titles begin with `Newsletter suggestion:` are also crawled on the next scheduled run.
